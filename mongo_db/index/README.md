...
인덱스는 데이터베이스에서 정보를 찾는 과정을 빠르게 하기 위한 도구로, 책의 목차나 사전의 색인과 비슷한 역할을 합니다. 

전체 내용을 살펴보는 대신 특정 내용을 가리키는 정렬된 리스트를 확인함으로써 더 빠르게 원하는 정보를 찾을 수 있습니다.

인덱스를 사용하지 않은 쿼리를 컬렉션 스캔이라고 합니다. (컬렉션 스캔은 모든 문서를 읽어야 하므로 성능이 떨어집니다.)

## 인덱스 적용 전

`explain()` 함수를를 사용하여 쿼리의 실행 계획을 확인할 수 있습니다.

```shell
# 속도 비교를 위한 100만개의 데이터 생성
for (i = 0; i < 1000000; i++) {
...
# 필요한 부분만 추출
executionStats: {
    nReturned: 1, # 반환 받은 결과의 개수
    executionTimeMillis: 359, # 쿼리하는데 걸린 시간
    totalDocsExamined: 1000000, # 쿼리를 실행하면서 살펴본 문서의 수
}
```

MongoDB는 사용자명이 고유함을 알지 못하므로 일치하는 항목을 찾으려면 컬렉션 내 모든 문서를 검색해야 합니다.

MongoDB가 쿼리에 효율적으로 응답하게 하려면 애플리케이션의 모든 쿼리 패턴에 인덱스를 사용해야 합니다.

> 필자 의문점 : 가장 자주 사용되고 성능에 큰 영향을 미치는 쿼리 패턴만 인덱스를 생성하는게 좋지않을까?

> 쿼리 패턴(Query Pattern)
> 
> 데이터베이스에 요청되는 특정 유형의 쿼리를 의미합니다. 즉, 단순한 개별 쿼리가 아니라 비슷한 유형이나 구조를 가진 쿼리의 집합을 말합니다.  
> 사용자의 이름으로 사용자 정보를 조회하는 쿼리, 특정 날짜 범위 내에서 이벤트를 검색하는 쿼리 이런 예시들은 각각 다른 쿼리 패턴을 나타냅니다.

## 인덱스 생성

몽고DB에서의 인덱스는 전형적인 관계형 데이터베이스의 인덱스와 거의 동일하게 작동합니다.
...
executionStats: {
    nReturned: 1,
    executionTimeMillis: 6, # 쿼리하는데 걸린 시간
    totalDocsExamined: 1,
}
```

359 -> 6 으로 쿼리 시간이 줄어들었습니다.

### 단점

인덱싱된 필드를 변경하는 쓰기(INSERT, UPDATE, DELETE) 작업이 더 오래 걸리게 됩니다. 

인덱싱된 필드(즉, 예시에서는 username) 데이터가 변경될 때마다 인덱스도 변경되어야 하기 때문입니다. 다른 필드만 변경하거나 삭제하는 경우 인덱스로 인한 성능저하는 없습니다.

### 인덱싱 기준

어떤 필드가 인덱싱하기에 적합한지 신중하게 결정해야 합니다.

해당 쿼리가 자주 쓰이거나 병목이 된다면 인덱스를 생성하는 것을 고려해야 합니다.

## 복합 인덱스

상당 수의 쿼리 패턴은 두개 이상의 키를 기반으로 인덱스를 작성해야 합니다. 

복합 인덱스는 2개 이상의 필드로 구성된 인덱스로 쿼리에서 정렬 방향이 여러 개이거나 검색 조건에 여러 개의 키가 있을 때 유용 합니다.
...
age로 정렬한 후 username으로 정렬하는데 이럴 때 username에 의한 완전 정렬은 별로 도움이 되지 않습니다.

정렬을 최적화하려면 "age"와 "username"을 기반으로 `복합 인덱스`를 생성해야 합니다.
...
> Q. { "age" : 1, "username" : 1 } 과 { "gender" : 1, "username" : 1} 인덱스가 존재하는 경우 { "age" : 1, "gender" : 1 }가 인덱스를 탈까?
> 
